    ...
    def AddRelation(self):
        ...
        try:
            if not str.isdigit(from_):
                raise Exception(
                    'Введите вершину "Откуда" - положительное целое число')
            if not str.isdigit(to_):
                raise Exception(
                    'Введите вершину "Куда" - положительное целое число')
    ...
    def Create(self):
        ...
        try:
            if not str.isdigit(self.vertCount.text()):
                raise Exception(
                    'Введите целочисленное положительное количество вершин')
            if not str.isdigit(self.relCount.text()):
                raise Exception(
                    'Введите челочисленное положительное количество связей')
            ...
            for i in range(1, relationNumb+1):
                ...
                # Если получилась петля или уже дублирующая связь
                # (вне зависимости от направления), меняем числа
                while from_ == to_ or to_ in list(self.g.adj[from_]) or from_ in list(self.g.adj[to_]):
                    ...
                    if vertexNumb*(vertexNumb-1)/2 < relationNumb:
                        raise Exception(
                            'Столько уникальных ребер\
                            не может быть при указанном количестве вершин'
                            )
            ...
            matrix = np.zeros(
                (self.g.number_of_nodes(), self.g.number_of_nodes()))

            for node_from in self.g.nodes():
                for node_to in self.g.nodes():
                    if node_to in self.g.adj[node_from]:
                        matrix[node_from-1][node_to -
                                            1] = self.g[node_from][node_to]['weight']
    ...
    def DeleteVertex(self):
        ...
        try:
            if not str.isdigit(deleteNumber):
                raise Exception(
                    'Введите номер удаляемой вершины - положительное целое число')
    ...
    def RunFordAlg(self):
        try:
            if not str.isdigit(self.fordStartVertex.text()):
                raise Exception(
                    'Введите номер вершины - положительное целое число')
            ...
            while not q.empty():
                ...
                for neighbour in list(self.g.adj[current_node]):
                    new_dist = distances[str(
                        current_node)] + self.g[current_node][neighbour]['weight']
                    if neighbour in nodes and neighbour not in q.queue:
                        q.put(neighbour)
